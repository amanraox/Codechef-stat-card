...
export function buildCard(
  ...
  // CodeChef icon + text — top-right, aligned with name baseline
  const ccTextX = w - PADDING;
  const ccIconSize = 14;
  const ccIconX = ccTextX - 68; // text is ~60px wide, icon before it
  const ccIconY = PADDING + 7;
  const ccScale = ccIconSize / 24;
  children.push(
    new Item("g", { transform: `translate(${ccIconX},${ccIconY}) scale(${ccScale})` }, [
      new Item("path", {
  ...
  children.push(
    text("CodeChef", ccTextX, PADDING + 20, {
      size: 13,
      fill: theme.subtext,
      weight: "600",
      anchor: "end",
